...
import edu.northeastern.ccs.im.Utils.AgencyUtils;
...
public abstract class Prattle {
        ...
        for (ClientRunnable tt : active) {
            ...
            if (tt.isInitialized()) {
                if (!tt.isAgency()) {
                    ...
                    if (AgencyUtils.isTarget(message.getName(), tt))
                        tt.enqueueMessage(message);
                }
            }
        }
    ...
    /**
     * Broadcast a given group message to all the other IM clients in the same group currently on the
     * system. This message _will_ be sent to the client who originally sent it.
     *
     * @param message   Message that the client sent.
     * @param groupName Group that the message sent to.
     */
    public static void broadcastGroupMessage(String groupName, Message message) {
        CRUD crud = new CRUD();
        List groupMembers = crud.readFileFromGroupMember(groupName);
        // Loop through all of our active threads
        for (ClientRunnable tt : active) {
            // Do not send the message to any clients that are not ready to receive it.
            if ((tt.isInitialized() && groupMembers.contains(tt.getName())) || AgencyUtils.isTarget(groupName, tt)) {
                tt.enqueueMessage(message);
            }
        }
    }

    public static void sendPrivateMessage(String receiver, String sender, Message message) {
        CRUD crud = new CRUD();
        List clientsList = new ArrayList<String>();
        for (ClientRunnable tt : active) {
            clientsList.add(tt.getName());
            if ((tt.isInitialized() && receiver.equalsIgnoreCase(tt.getName())) ||
                    AgencyUtils.isTarget(sender, receiver, tt)) {
                tt.enqueueMessage(message);
                crud.addReceivedMessage(message.getText(), receiver, sender, tt.getIP());
            }
        }
        if (!clientsList.contains(receiver)) {
            crud.initMissedMessageFile(receiver);
            crud.addMissedMessage(message.getText(), receiver, sender, "");
        }
    }

    public static void sendMissMessageToUser(String username) throws IOException{
        CRUD crud = new CRUD();
        List<ArrayList<String>> missedMessages = crud.readMissedMessage(username);
        if (missedMessages == null) {
            return;
        }
        for (ClientRunnable tt : active) {
            if (tt.isInitialized() && username.equalsIgnoreCase(tt.getName())) {
                for (int i = 0; i < missedMessages.size(); i++) {
                    String sender = missedMessages.get(i).get(0);
                    String content = missedMessages.get(i).get(1);
                    Message missMsg = Message.makeBroadcastMessage(sender + " to you offline",
                            content);
                    sendPrivateMessage(username, sender, missMsg);
                }
            }
        }
        try {
        ...
        crud.cleanMissMessage(username);

    }

    public static void recallMessage(String receiver, String sender, String content) throws IOException {
        CRUD crud = new CRUD();
        crud.deleteRecallMessage(receiver, sender, content);
    }
    ...
    public static void main(String[] args) throws IOException {
        String loggerSwitch = args == null ? null : args.length == 0 ? null : "off";
        if (loggerSwitch != null && loggerSwitch.matches("off")) {
        ...
        crud.initAllGroupsDirectory();
        crud.initMemberProfilesDirectory();
