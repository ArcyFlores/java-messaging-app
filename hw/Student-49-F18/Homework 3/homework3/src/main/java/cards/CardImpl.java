 ...
 */
   ...
   * @return true if they are the same type of card, false otherwise.
   */
  public boolean sameTypeCard(Card c) {
    if(this == c) {
      return true;
    }
    return this.getRank().getName().equals(c.getRank().getName())
            && this.getSuit().getSymbol() == c.getSuit().getSymbol();
  }
}
